...
Game::Game(): human(humanHand), computer(computerHand)
{
	players[0] = &human;
	players[1] = &computer;
}
...
Reference: none
********************************************************************* */
bool Game::checkHandForTile(Player* p, string target) {
...
********************************************************************* */
void Game::saveGameState(string filename, Hand humanHand, Hand computerHand, Stock& gameStock, Tournament& tournament, Layout& layout, Round& currentRound) {
...
********************************************************************* */
void Game::loadGameState(string filename, Player* human, Player* computer, Stock& gameStock, Tournament& tournament, Layout& layout, Round& currentRound) {
...
void Game::newLoadGameState(string filename)
{
	bool passVal = false;
	...
	while (getline(is, line))
	{
		...
		if (line.find("Tournament Score:") != string::npos)
		{
			...
			{
				string value = line.substr(colonPos + 1);
				gameTournament.setTournScore(stoi(value));
			}
		}

		// Round Number
		else if (line.find("Round No.:") != string::npos)
		{
			size_t colonPos = line.find(":");
			if (colonPos != string::npos)
			{
				string value = line.substr(colonPos + 1);
				gameRound.setRoundNum(stoi(value));
			}
		}
		...
		else if (line.find("Computer:") != string::npos)
		{
			...
			if (getline(is, line) && line.find("Score:") != string::npos)
			{
				...
				{
					string value = line.substr(colonPos + 1);
					gameTournament.setComputerScore(stoi(value));
				}
			}
		}
		...
		else if (line.find("Human:") != string::npos)
		{
			...
			if (getline(is, line) && line.find("Score:") != string::npos)
			{
				...
				{
					string value = line.substr(colonPos + 1);
					gameTournament.setHumanScore(stoi(value));
				}
			}
		}
		...
		else if (line.find("Previous Player Passed:") != string::npos)
		{
		...
		else if (line.find("Next Player:") != string::npos)
		{
			...
			if (colonPos != string::npos)
			{
				string nextName = line.substr(colonPos + 1);

				if (nextName.find("Computer") != string::npos)
				{
					gameRound.setCurrentPlayer(1);
					gameRound.setNextPlayer(0);
				}
				else
				{
					gameRound.setCurrentPlayer(0);
					gameRound.setNextPlayer(1);
				}

				if (passVal)
					gameRound.setPassed(gameRound.getNextPlayer());
				else
					gameRound.resetPass(gameRound.getNextPlayer());
			}
		}
	}
	...
	roundInitialized = true;  // important so runTournament doesn’t redeal
}
/**
 ...
 */
void Game::initSave(Hand humanHand, Hand computerHand, Stock& gameStock, Tournament& tournament, Layout& layout, Round& currentRound)
{
	int inp;
	do
	{
...
********************************************************************* */
void Game::addTotalPoints(Player& winner, Player& loser, Tournament& gameTournament) {
	int total = 0;
...
Reference: none
********************************************************************* */
void Game::tiePoints(Player* Human, Player* Computer, Tournament& gameTournament)
{
...
Reference: none
********************************************************************* */
string Game::obtainEngine(Round gameRound, Player* players[], Stock& gameStock)
{
	string engine = "";
...
Reference: none
********************************************************************* */
void Game::firstTurn(string engine, Layout& layout, Player* players[], Round& gameRound)
{
...
void Game::runTournament(Tournament &gameTournament)
...
void Game::runRound(Round& gameRound, Layout& layout, Tournament& gameTournament, Player *players[], LayoutView &gameView)
{
	bool placedTile[2] = { false, false };
